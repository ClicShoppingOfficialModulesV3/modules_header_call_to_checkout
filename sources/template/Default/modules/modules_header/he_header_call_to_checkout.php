<?php
/**
 * he_header_call_to_checkout.php
  ...
  class he_header_call_to_checkout {
    ...
    public function execute() {
      ...
      if ($CLICSHOPPING_ShoppingCart->getCountContents() > 0 && !isset($_GET['Checkout'])) {
        $call_to_checkout = '<div class="alert alert-danger text-md-center headerCallToCheckoutAlertDanger" role="alert"><i class="fas fa-flag fa-lg"></i>' . CLICSHOPPING::getDef('text_call_to_checkout', ['count_content' =>$CLICSHOPPING_ShoppingCart->getCountContents()]) . '&nbsp;&nbsp;' . HTML::button(CLICSHOPPING::getDef('button_call_to_checkout'), 'fas fa-thumbs-o-up fa-lg', CLICSHOPPING::link('index.php', 'Checkout&Shipping'), 'primary', NULL, 'btn-success') . '</div>';
      }else{
        $call_to_checkout = '';
      }
